...
class App {
        ...
        try{ 
            if(req.session.schoolCode){
                ...
                res.render('new-Parent', {title : 'New Parent', schoolAdmin : schoolAdmin,
                        error : req.flash('error'), parent_active : "active", student : student})
            }else{
                res.redirect(303, '/school')
            }
        }catch(err){
    ...
    postParents = async (req , res , next) => { 
        try{ 
		    if(req.session.schoolCode){	 
                const schoolAdmin = await SchoolAdmin.findOne({schoolCode : req.session.schoolCode})
                const {name, gender, email, number, role, children} = req.body
    ...
    getParentComplete = async (req, res, next) => {
        try{ 
            if(req.session.schoolCode){
                ...
                if(parent){
                    res.render("complete-parentreg", {title : "Upload Image", parent : parent, 
        ...
        try{ 
            if(req.session.schoolCode){
                ...
                if(validParent){
                    ...
                    res.render('singleparent' , { title  : "Parent", parentDB: validParent, student: student,
    ...
    // confirmParent = async (req, res, next) => {
    //     try{ 
    //         if(req.session.schoolCode){
    //             const schoolAdmin = await SchoolAdmin.findOne({schoolCode : req.session.schoolCode})

    //             const parent = await Parent.findOne({school : schoolAdmin._id})
    //             if(parent){
    //                 res.json({message : "A Parent has been chosen for this student.", button : true})
    //             }else{
    //                 res.json({message : "Good!", button : false})
    //             }
    //         }else{
